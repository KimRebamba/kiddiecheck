...
<div class="d-flex align-items-center mb-3">
  <h1 class="h4 mb-0">Family Test for {{ $test->student->name }}</h1>
  <div class="ms-auto">
    <a class="btn btn-sm btn-outline-secondary" href="{{ route('family.index') }}">Back to Dashboard</a>
  </div>
</div>

@php
  // Prefer controller-provided values; fallback to computing here
  $totalQuestions = $totalQuestions ?? (\App\Models\Domain::with('questions')->count() ? \App\Models\Domain::with('questions')->get()->sum(fn($d)=>$d->questions->count()) : null);
  $answeredCount = $answeredCount ?? $test->responses->count();
  $progressPct = $progressPct ?? ($totalQuestions ? round(($answeredCount / max(1,$totalQuestions)) * 100) : null);
@endphp
...
<div class="card mb-3">
  <div class="card-body">
    <div class="d-flex align-items-center justify-content-between">
      ...
      <span class="badge bg-primary">Question {{ $index + 1 }} of {{ $domain->questions->count() }}</span>
    </div>
  </div>
</div>
...
<div class="card">
  <div class="card-body">
    ...
    <form method="post" action="{{ route('family.tests.question.submit', [$test->id, $domain->id, $index]) }}" class="mt-3">
      @csrf
    ...
    <div class="mt-3 d-flex gap-2">
      <form method="post" action="{{ route('family.tests.pause', $test->id) }}">
        @csrf
        <button type="submit" class="btn btn-outline-secondary">Pause</button>
      </form>
      <form method="post" action="{{ route('family.tests.cancel', $test->id) }}">
        @csrf
        <button type="submit" class="btn btn-outline-danger">Cancel</button>
      </form>
      <form method="post" action="{{ route('family.tests.terminate', $test->id) }}">
        @csrf
