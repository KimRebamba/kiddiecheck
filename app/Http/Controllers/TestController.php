...
class TestController extends Controller
{
    ...
    public function complete($testId)
    {
        ...
        if (!$test) {
            return redirect()->route('dashboard')->with('error', 'Test not found.');
        }
        
        // Ensure test is completed
        if ($test->status !== 'completed') {
            return redirect()->route('test.review', $testId)
                ->with('error', 'Test must be completed first.');
        }
        
        // Use the ECCD scoring service
        $scoring = new EccdScoring();
        $testStandardScore = $scoring->scoreTest($test);
        
        if (!$testStandardScore) {
            return redirect()->route('test.review', $testId)
                ->with('error', 'Unable to calculate scores. Please check test data.');
        }
        
        $student = $test->student;
        
        // Get domain scores from the scoring results
        $domainScores = DB::table('test_domain_scaled_scores')
            ->join('domains', 'test_domain_scaled_scores.domain_id', '=', 'domains.domain_id')
            ->where('test_domain_scaled_scores.test_id', $testId)
            ...
            ->map(function ($score) {
                return [
                    ...
                    'icon' => $this->domainIcons[$score->name] ?? '📋'
                ];
            });
        ...
        return view('test.complete', [
            ...
            'standardScore' => $testStandardScore->standard_score,
            'sumScaledScores' => $testStandardScore->sum_scaled_scores,
            'interpretation' => $testStandardScore->interpretation
        ]);
    }
}
